...
thirty.addEventListener("click", () => setTimer(30, thirty, sixty));
sixty.addEventListener("click", () => setTimer(60, sixty, thirty));
beg.addEventListener("click", () => setDifficulty(1, beg, pro));
pro.addEventListener("click", () => setDifficulty(2, pro, beg));
...
function setTimer(value, selected, other) {
    timer = value;
    factor = value === 30 ? 2 : 1;
    updateLimitColor(selected, other);
    time.innerText = timer;
}

function setDifficulty(level, selected, other) {
    difficulty = level;
    displayTest(difficulty);
    updateLimitColor(selected, other);
}

function updateLimitColor(active, inactive) {
    active.classList.add("yellow");
    inactive.classList.remove("yellow");
}
...
function restartTest() {
    ...
    toggleLimitVisibility(true);
}

function startTimer() {
    toggleLimitVisibility(false);
...
function toggleLimitVisibility(visible) {
    const visibility = visible ? "visible" : "hidden";
    [thirty, sixty, beg, pro].forEach(element => {
        element.style.visibility = visibility;
    });
}

function displayScore() {
    const accuracy = wordsSubmitted ? Math.floor((wordsCorrect / wordsSubmitted) * 100) : 0;
...
function updateCurrentWord() {
    ...
    const currentWord = currentSpan.innerText;

    if (currentWord.startsWith(wordEntered)) {
        applyColor(currentSpan, "current");
    } else {
        applyColor(currentSpan, "wrong");
    }
}
...
function checkWord() {
    ...
    if (currentSpan.innerText.trim() === wordEntered) {
        applyColor(currentSpan, "correct");
        wordsCorrect++;
        cw.innerText = wordsCorrect;
    } else {
        applyColor(currentSpan, "wrong");
    ...
    if (wordNo > 40) {
        ...
        applyColor(document.getElementById(`word ${wordNo}`), "current");
    }
}

function applyColor(element, status) {
    element.className = status;
}

function displayTest(level) {
    wordNo = 1;
    testItem.innerHTML = "";
    const words = generateWords(level);
    ...
    applyColor(document.getElementById("word 1"), "current");
}
...
function generateWords(level) {
    ...
    const wordPool = level === 1 ? basicWords : advancedWords;
